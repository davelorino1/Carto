...
#     check_df = check.to_dataframe()
    ...
    check = client.query(
        ...
        CREATE OR REPLACE TABLE cart-dai-sandbox-nonprod-c3e7.{data_scientist}.regularization_process_baseline_statistics_avg_price_per_unit AS 
            with step_one AS (
                SELECT 
                    trading.booking_and_asset_number AS campaign_id,
                    #MIN(BusinessDate) AS earliest_date, 
                    ...
                    ass_pre_campaign.SiteNumber AS Site,
                    ass_pre_campaign.Article, 
                    ass_pre_campaign.ArticleDescription,
                    COUNT(DISTINCT DATE_TRUNC(ass_pre_campaign.BusinessDate, WEEK(WEDNESDAY))) AS weeks_count,
                    SUM(ass_pre_campaign.TotalAmountIncludingGST) AS total_sales_amount,
                    SUM(ass_pre_campaign.RetailQuantity) AS total_units
                ...
                AND ass_pre_campaign.BusinessDate <= DATE_ADD(media_end_date_global_var, INTERVAL -1 WEEK)
                AND ass_pre_campaign.SalesOrganisation = '1005'
                AND ass_pre_campaign.TotalAmountIncludingGST > 0
                AND skus.sku IS NOT NULL
                AND skus.sku <> ""
                GROUP BY 1,2,3,4
            )
            SELECT 
                ...
                total_sales_amount, 
                total_units, 
                SAFE_DIVIDE(total_sales_amount , total_units) AS avg_ppu
            FROM step_one 
        ; 
    ...
    check = client.query(
        ...
        CREATE OR REPLACE TABLE cart-dai-sandbox-nonprod-c3e7.{data_scientist}.regularization_process_campaign_period_avg_price_per_unit AS 
            with step_one AS (
                SELECT 
                    trading.booking_and_asset_number AS campaign_id,
                    trading.media_start_date,
                    trading.media_end_date,
                    ass_campaign_period.Article,
                    ass_campaign_period.ArticleDescription,
                    ass_campaign_period.SiteNumber AS Site,
                    SUM(ass_campaign_period.TotalAmountIncludingGST) AS sales_amount,
            ...
            SELECT *, 
                SAFE_DIVIDE(sales_amount , total_units) AS avg_ppu 
            FROM step_one
        ;

        CREATE OR REPLACE TABLE cart-dai-sandbox-nonprod-c3e7.{data_scientist}.regularization_process_historical_vs_campaign_price_per_unit AS 
            SELECT 
                historical.campaign_id, 
                historical.Site,
                historical.Article, 
                historical.ArticleDescription,
                historical.weeks_count AS historical_n_weeks,
                historical.avg_ppu AS historical_avg_ppu, 
                campaign.avg_ppu AS campaign_avg_ppu
            FROM cart-dai-sandbox-nonprod-c3e7.{data_scientist}.regularization_process_baseline_statistics_avg_price_per_unit historical 
            LEFT JOIN cart-dai-sandbox-nonprod-c3e7.{data_scientist}.regularization_process_campaign_period_avg_price_per_unit campaign
                ON historical.Article = campaign.Article
                AND historical.Site = campaign.Site
        ;
...
#     chime.success() 
    ...
    def run_model(sigma_value, merged_df, matched_pairs):
        ...
        for _, pair in matched_pairs.iterrows():
            ...
            test_store_posteriors = merged_df[merged_df['store'] == test_store]['transformed_campaign_week_uplift'].values
            control_store_posteriors = merged_df[merged_df['store'] == control_store]['transformed_campaign_week_uplift'].values
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                transformed_incremental_uplift = (test_store_posteriors - control_store_posteriors)
        ...
        with pm.Model() as model:
            ...
            #     sigma=sigma_obs,
            #     observed=posterior_incremental_differences_df['standardized_incremental_uplift'].values
            # )
    ...
    def optimize_sigma(merged_df, matched_pairs, initial_sigma=0.005, initial_step=0.005, max_iterations=80):
