...
for campaign_id in campaign_ids_list:
    ...
    check = client.query(
        ...
        SET media_start_date_global_var = (SELECT DISTINCT media_start_date FROM gcp-wow-cart-data-dev-d4d7.davide.instore_screens_campaigns_june_2023_onwards_2 WHERE booking_and_asset_number = '{campaign_id}');
        SET media_end_date_global_var = (SELECT DISTINCT media_end_date FROM gcp-wow-cart-data-dev-d4d7.davide.instore_screens_campaigns_june_2023_onwards_2 WHERE booking_and_asset_number = '{campaign_id}');
    ...
    def run_model(sigma_value, merged_df, matched_pairs):
        ...
        with pm.Model() as model:
            ...
            mu_pop = pm.HalfNormal('mu_pop', sigma=0.005)
    ...
    def optimize_sigma(merged_df, matched_pairs, initial_sigma=0.004, initial_step=0.001, max_iterations=80):
